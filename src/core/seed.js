...
export const seed = {
  ...
  capabilities:[
    ...
    {id:'model.reason',name:'Suy luận',authority:'REASON',status:'ACTIVE',agents:['research-agent','knowledge-agent','data-agent','management-agent','executive-agent','learning-agent','data-intake-agent','data-quality-agent','access-governance-agent','system-governance-agent']},
  ...
  agents:[
    ...
    {id:'data-intake-agent',name:'AI Nhập dữ liệu',owner:'System Administration',accountableRole:'SYSTEM_ADMIN',capabilities:['data.intake','model.reason'],defaultActions:['READ_FILE','MAP_FIELDS','ANALYZE','CREATE_DRAFT','WRITE_STAGING'],status:'ACTIVE'},
  ...
  workItems:[
    ...
    {id:'WORK-DATA-INTAKE',title:'Tiếp nhận và chuẩn hóa dữ liệu đầu vào',purpose:'Con người và AI cùng tiếp nhận, đọc, chuẩn hóa và đề xuất dữ liệu staging trước khi người có thẩm quyền commit',humanOwner:'USR-ADMIN',scope:{type:'SYSTEM',id:'ANU2'},domain:'OIS',priority:'HIGH',status:'IN_PROGRESS',deadline:'2026-12-31',expectedOutcome:'Dữ liệu/tài liệu được đọc, phân loại, kiểm tra và chuẩn bị staging; mọi thay đổi master data cần human checkpoint',artifactIds:[],assignments:[
      {id:'ASG-INTAKE-1',agentId:'data-intake-agent',task:'Đọc dữ liệu đa phương tiện, nhận yêu cầu bằng lời nói, đề xuất mapping và bản ghi staging',domain:'OIS',resourceScope:'DATA_INTAKE',dataScope:'*',validUntil:null,humanCheckpoint:'BEFORE_COMMIT',delegatedBy:'USR-ADMIN',capabilities:['data.intake','model.reason'],actions:['READ_FILE','MAP_FIELDS','ANALYZE','CREATE_DRAFT','WRITE_STAGING'],status:'READY'},
      {id:'ASG-INTAKE-2',agentId:'data-quality-agent',task:'Kiểm tra chất lượng, trùng lặp, thiếu dữ liệu và bất thường trước commit',domain:'OIS',resourceScope:'DATA_INTAKE',dataScope:'*',validUntil:null,humanCheckpoint:'BEFORE_COMMIT',delegatedBy:'USR-ADMIN',capabilities:['data.quality','model.reason'],actions:['READ','ANALYZE','FLAG_ISSUE'],status:'READY'}
    ],outputs:[],createdAt:'2026-09-24T03:30:00Z'},
